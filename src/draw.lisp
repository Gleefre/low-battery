...
(defun draw-cell (items x y)
  (when (and *editing* (numberp x) (numberp y))
    ...
    (s+:with-color (s:+black+ :stroke)
      (s:rect 10 10 80 80))
    (when (equal (list x y) (editing-cell *editing*))
      (s:with-pen (s:make-pen :weight 10 :stroke s:+magenta+)
        (s:rect 5 5 90 90)))
    (when (equal (list *room* x y) (ref *editing*))
      (s:with-pen (s:make-pen :weight 10 :stroke s:+blue+)
        (s:rect 0 0 100 100))))
              ...
              (if *with-images*
                  (progn
                    (s+:with-color (s:+black+)
                      (s:rect 15 35 60 30))
                    (s:image (s:load-resource (pic (format nil "battery-~a.png" (car arg))))
                             0 0 *unit* *unit*))
                  (progn
                    (s:rect 10 30 80 60)
                    (s+:with-color (s:+green+)
                      (s:rect 20 35 (* 20 (car arg)) 50)))))
             (:text
              (if *with-images*
                  (progn
                    (s+:with-color (s:+black+)
...
(defun draw-action-buttons (width height)
  (macrolet ((action-button ((x y name) &body press-body)
               `(named-button ((+ 20 (* 400 ,x)) (+ 20 (* 200 ,y)) 360 160 ,name) (:press)
                  ,@press-body)))
    (s+:with-fit ((* *unit* 12) (* *unit* 8) width height)
      (s:with-font (s:make-font :size 35 :align :center)
        (action-button (2 3 "Save room")
          (save-room (room-filename *room*)))
        (action-button (2 2 "Reset room")
          (reset-room *room*))
        (action-button (2 1 "(!) Reload room")
          (load-room (room-filename *room*)))
        (when (editing-cell *editing*)
          (action-button (0 1 "Move room origin")
        ...
        (when (and (editing-cell *editing*)
          ...
          (action-button (0 3 #.(format nil "Set portal~%to ref"))
            (let ((cell (find :portal (apply #'cell (editing-cell *editing*)) :key #'car)))
              (setf (cdr cell) (ref *editing*))))
          (when (let ((*room* (car (ref *editing*))))
                  (find :portal (apply #'cell (cdr (ref *editing*))) :key #'car))
            (action-button (1 3 #.(format nil "Set portal~%from ref"))
              (let ((cell (let ((*room* (car (ref *editing*))))
                            (find :portal (apply #'cell (cdr (ref *editing*))) :key #'car))))
                (setf (cdr cell)
        ...
        (when (editing-cell *editing*)
          ...
          (action-button (1 1 #.(format nil "Move hero~%to cell"))
            (setf (x *hero*) (car (editing-cell *editing*))
                  (y *hero*) (cadr (editing-cell *editing*)))))
        (when (ref *editing*)
          (action-button (1 2 "Delete ref")
            (setf (ref *editing*) nil)))
        (action-button (0 0 "Next room")
          (setf (game-room *game*) (next-room)))
        (action-button (1 0 "Reset hero")
